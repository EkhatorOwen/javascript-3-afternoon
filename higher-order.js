...
*/
//Code Here
let postTaxPrices  = prices.map((element,index,arr)=>{return 1.07*element} );
...
let totalPopulation   = populations.reduce((sum,curElement) => {return sum+curElement})
...
//Code Here
let myStrongest  = monstersInYourPocket.filter((element,index,arr)=>{return element.CP>200 })
...
*/
var sum = 0;
let ordersTotal = orders.map((element,index,arr)=>{
  sum = element.price + element.tax
})
...
*/
var sumd = 0;
let bobsTotal = purchases.reduce((sum,curElement,index,arr)=>{
    
  if(curElement.owner=='Bob')
  {
    sumd += curElement.price;
  }
    return sumd  
})
